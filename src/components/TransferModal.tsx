    ...
    <Modal
      ...
      footer={(
        <div className="w-full flex justify-end gap-2">
          <button onClick={onClose} className="px-3 py-1.5 rounded border border-[#2D3139] text-[#94A3B8] hover:text-white hover:bg-[#2D3748] cursor-pointer">{t('common.cancel')}</button>
          <button onClick={handleAction} disabled={busy} className="px-4 py-1.5 rounded bg-[#3B82F6] text-white font-semibold hover:bg-[#2563EB] disabled:opacity-50 cursor-pointer flex items-center gap-2">
      ...
      {isExport ? (
        <div className="space-y-2">
          {([
            ...
            <button key={value} onClick={() => setFormat(value)} className={`w-full p-3 rounded border text-left flex items-center gap-3 cursor-pointer ${format === value ? 'border-[#3B82F6] bg-[#1E293B]' : 'border-[#2D3139] bg-[#090B0E] hover:bg-[#1C2128]'}`}>
              <Icon className={`w-5 h-5 ${format === value ? 'text-[#38BDF8]' : 'text-[#94A3B8]'}`} />
              <span><span className="block text-white font-semibold text-xs">{title}</span><span className="block text-[#64748B] text-[11px] mt-0.5">{description}</span></span>
            </button>
          ))}
          {format === 'csv' && (
            <label className="block pt-3 text-[#94A3B8] text-xs">
        ...
        <div className="rounded border border-[#2D3139] bg-[#090B0E] p-4 text-[#94A3B8] text-xs leading-relaxed">
